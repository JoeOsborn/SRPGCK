...
public class ProxyMoveExecutor : MoveExecutor {
	override public void TemporaryMoveTo(PathNode pn, MoveFinished callback, float timeout=10.0f, bool knockedBack=false) {
		Vector3 src = temporaryPosition;
		UpdatePositions();
		callback(src, temporaryDestNode, true);
	}

	override public void IncrementalMoveTo(PathNode pn, MoveFinished callback, float timeout=10.0f, bool knockedBack=false) {
		MoveTo(pn, callback, timeout);
	}

	override public void MoveTo(PathNode pn, MoveFinished callback, float timeout=10.0f, bool knockedBack=false) {
	...
	public void UpdatePositions() {
		destNode = new PathNode(character.transform.position, null, 0);
